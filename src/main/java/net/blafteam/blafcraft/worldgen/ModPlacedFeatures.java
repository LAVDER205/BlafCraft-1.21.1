...
public class ModPlacedFeatures { // where to place and how many
    // CF -> PF -> BM
    ...
    public static void bootstrap(BootstrapContext<PlacedFeature> context) {
        ...
        register(context, END_BISMUTH_ORE_PLACED_KEY, configuredFeatures.getOrThrow(ModConfiguredFeatures.END_BISMUTH_ORE_KEY),
                ModOrePlacement.commonOrePlacement(12, HeightRangePlacement.uniform(VerticalAnchor.absolute(-64), VerticalAnchor.absolute(80))));
                // pCount - how many ores in a chunk
                // uniform - same chance at any y level, triangle - highest chance in the middle

        // Actual mod
        register(context, SCULK_ORE_PLACED_KEY, configuredFeatures.getOrThrow(ModConfiguredFeatures.OVERWORLD_SCULK_ORE_KEY),
                ModOrePlacement.commonOrePlacement(30, HeightRangePlacement.triangle(VerticalAnchor.absolute(-64), VerticalAnchor.absolute(80))));
    }

    private static ResourceKey<PlacedFeature> registerKey(String name) {
        return ResourceKey.create(Registries.PLACED_FEATURE, ResourceLocation.fromNamespaceAndPath(BlafCraft.MODID, name));
    }
